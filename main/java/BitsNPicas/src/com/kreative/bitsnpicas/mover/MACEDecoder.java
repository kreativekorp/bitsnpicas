...
public class MACEDecoder {
	private static final int[] MAJOR_INDEX_DELTA = {
		-13, 8, 76, 222, 222, 76, 8, -13
	};
	
	private static final int[][] MAJOR_LEVEL_DELTA = {
	...
	private static final int[] MINOR_INDEX_DELTA = {
		-18, 140, 140, -18
	};
	
	private static final int[][] MINOR_LEVEL_DELTA = {
		{ 64, 216, -217, -65 },
		{ 67, 226, -227, -68 },
		{ 70, 236, -237, -71 },
		{ 74, 246, -247, -75 },
	...
	private static class MACEState {
		int index; int factor;
		int prev2; int prev1; int level;
		int inOffset; int outOffset;
	}
	
	private static void decompressBits3(MACEState state, int b, short[] out, int[] indexDelta, int[][] levelDelta, int channels) {
		int curr = levelDelta[(state.index & 0x7F0) >> 4][b] + state.level;
		if (curr > 32767) curr = 32767;
		if (curr < -32768) curr = -32767;
		state.level = curr - (curr >> 3);
		out[state.outOffset] = (short)curr;
		state.outOffset += channels;
		state.index += indexDelta[b] - (state.index >> 5);
		if (state.index < 0) state.index = 0;
	}
	
	private static void decompressBits6(MACEState state, int b, short[] out, int[] indexDelta, int[][] levelDelta, int channels) {
		int curr = levelDelta[(state.index & 0x7F0) >> 4][b];
		if ((state.prev1 ^ curr) >= 0) {
			state.factor += 506;
			if (state.factor > 32767) state.factor = 32767;
		} else {
			state.factor -= 314;
			if (state.factor < -32768) state.factor = -32767;
		}
		curr += state.level;
		if (curr > 32767) curr = 32767;
		if (curr < -32768) curr = -32767;
		state.level = (curr * state.factor) >> 15;
		curr >>= 1;
		out[state.outOffset] = (short)(state.prev1 + state.prev2 - ((state.prev2 - curr) >> 2));
		state.outOffset += channels;
		out[state.outOffset] = (short)(state.prev1 + curr + ((state.prev2 - curr) >> 2));
		state.outOffset += channels;
	...
	private static void decompressStream3(byte[] in, short[] out, int channels) {
		MACEState state = new MACEState();
		for (int channel = 0; channel < channels; channel++) {
			state.index = 0; state.factor = 0;
			state.prev2 = 0; state.prev1 = 0; state.level = 0;
			state.inOffset = channel * 2; state.outOffset = channel;
			while (state.inOffset < in.length && state.outOffset < out.length) {
				int b = in[state.inOffset];
				decompressBits3(state, (b >> 0) & 7, out, MAJOR_INDEX_DELTA, MAJOR_LEVEL_DELTA, channels);
				decompressBits3(state, (b >> 3) & 3, out, MINOR_INDEX_DELTA, MINOR_LEVEL_DELTA, channels);
				decompressBits3(state, (b >> 5) & 7, out, MAJOR_INDEX_DELTA, MAJOR_LEVEL_DELTA, channels);
				b = in[state.inOffset+1];
				decompressBits3(state, (b >> 0) & 7, out, MAJOR_INDEX_DELTA, MAJOR_LEVEL_DELTA, channels);
				decompressBits3(state, (b >> 3) & 3, out, MINOR_INDEX_DELTA, MINOR_LEVEL_DELTA, channels);
				decompressBits3(state, (b >> 5) & 7, out, MAJOR_INDEX_DELTA, MAJOR_LEVEL_DELTA, channels);
				state.inOffset += channels * 2;
			}
		}
	}
	
	private static void decompressStream6(byte[] in, short[] out, int channels) {
		MACEState state = new MACEState();
		for (int channel = 0; channel < channels; channel++) {
			state.index = 0; state.factor = 0;
			state.prev2 = 0; state.prev1 = 0; state.level = 0;
			state.inOffset = channel; state.outOffset = channel;
			while (state.inOffset < in.length && state.outOffset < out.length) {
				int b = in[state.inOffset];
				decompressBits6(state, (b >> 5) & 7, out, MAJOR_INDEX_DELTA, MAJOR_LEVEL_DELTA, channels);
				decompressBits6(state, (b >> 3) & 3, out, MINOR_INDEX_DELTA, MINOR_LEVEL_DELTA, channels);
				decompressBits6(state, (b >> 0) & 7, out, MAJOR_INDEX_DELTA, MAJOR_LEVEL_DELTA, channels);
				state.inOffset += channels;
			}
		}
	}
	
	public static byte[] decompressMACE3(byte[] compressedData, int channels) {
		short[] decompressedData = new short[compressedData.length*3];
		decompressStream3(compressedData, decompressedData, channels);
		byte[] ret = new byte[decompressedData.length];
		for (int i = 0; i < decompressedData.length; i++) ret[i] = (byte)(decompressedData[i] >> 8);
		return ret;
	}
	
	public static byte[] decompressMACE6(byte[] compressedData, int channels) {
		short[] decompressedData = new short[compressedData.length*6];
		decompressStream6(compressedData, decompressedData, channels);
		byte[] ret = new byte[decompressedData.length];
		for (int i = 0; i < decompressedData.length; i++) ret[i] = (byte)(decompressedData[i] >> 8);
		return ret;
	}
